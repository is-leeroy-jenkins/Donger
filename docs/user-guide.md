...
tools = chat.get_supported_tools(<grok model>)
...
image = images.generate(
    prompt=(
        ...
        "that uses grok wrappers, SQLite persistence, document retrieval, and Google Cloud services."
    ),
    model="grok-2.5-flash-image",
    aspect="16:9",
    resolution="1K",
...
if image is not None:
    output_path = Path("docs/images/Donger-generated-architecture.png")
...
analysis = images.analyze(
    ...
    path="docs/images/Donger-architecture.png",
...
if edited is not None:
    output_path = Path("docs/images/Donger-architecture-edited.png")
...
```python
from grok import TTS

tts = TTS()

audio_bytes = tts.create_speech(
    text="Donger is ready to support text, image, audio, document, and data workflows.",
    model=<grok audio model>,
    voice="Kore",
    format="audio/wav",
)
...
output_path = tts.create_speech(
    text="This is a generated audio briefing from Donger.",
    filepath="outputs/Donger-briefing.wav",
    model=<grok audio model>,
    voice="Kore",
    format="audio/wav",
...
```python
from grok import Transcription
...
The `Files` class supports XAI file upload, file retrieval, file listing, document summarization,
document search, and file-backed workflows.

#### Upload a file

```python
from grok import Files
...
The `FileSearch` class manages grok File Search Store resources.

#### List file-search stores

```python
from grok import FileSearch
...
This example analyzes the Donger architecture image stored in the documentation assets folder.
...
The examples below show common Donger workflows by mode and by the wrapper class that supports each
...
**Backing class:** `grok.Chat`

Use Text mode when you want a grok-backed response to a prompt, with optional system instructions,
...
2. Select a supported grok text model.
...
3. Select an image-capable grok model.

4. Enter a detailed prompt, for example:

   ```text
   Create a dark-themed technical architecture diagram for a Streamlit application
   that uses grok class, local SQLite persistence, document retrieval, and
   XAI Cloud integrations.
   ```
...
through the `grok.Transcription`, `grok.Translation`, and `grok.TTS` classes.

#### Example: Transcribe an uploaded audio file

**Backing class:** `grok.Transcription`
...
**Backing class:** `grok.Translation`
...
**Backing classes:** `grok.Files`, `grok.Embeddings`, and local retrieval helpers

Use Document Q&A mode when you want Donger to answer questions using uploaded documents as context.
...
**Backing class:** `grok.Files`

Use Files mode for grok file upload, file metadata, document summarization, document search, and
...
**Backing class:** `grok.FileSearch`

Use File Search Stores mode to create, retrieve, list, delete, and manage grok file-search stores.
...
**Backing class:** `grok.CloudBuckets`

Use Google Cloud Buckets mode when Donger needs to interact with Google Cloud Storage buckets and
objects.
...
The following workflow combines several Donger modes:
